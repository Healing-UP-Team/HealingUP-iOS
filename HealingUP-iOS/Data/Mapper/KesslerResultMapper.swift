...
extension KesslerResultEntity {
  func map() -> KesslerResult {
    return KesslerResult(
      ...
      stressLevel: StressLevel(rawValue: stressLevel?.rawValue ?? "")!,
      createAt: createAt?.toDate() ?? Date())
  }
}

extension KesslerResult {
  func map() -> KesslerResultEntity {
    return KesslerResultEntity(
      id: id.uuidString,
      userId: userId,
      stressLevel: stressLevel,
      createAt: String(createAt.timeIntervalSince1970))
  }
}
